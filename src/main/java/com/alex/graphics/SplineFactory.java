...
// http://www.java2s.com/Code/Java/2D-Graphics-GUI/Asplinefactoryinstance.htm
...
public class SplineFactory {
	/**
	 * Create a Bezier spline based on the given control points. The generated curve
	...
	/**
	 * Create a Catmull-Rom spline based on the given control points. The generated
	 * curve starts in the first control point and ends in the last control point.
	 ...
	 * @param spline
	 * @param color
	 * @param spread 0 > down 0 <= horizontal
	 */
	public static void paintSpline(Graphics g, double[] spline, Color color, int spread) {
		if (spread == 0) {
			double xLast = spline[0];
			double yLast = spline[1];
			// double zLast = spline[2];
			g.setColor(color);
			for (int i = 3; i < spline.length; i += 3) {
				g.drawLine((int) xLast, (int) yLast, (int) spline[i], (int) spline[i + 1]);
				xLast = (int) spline[i];
				yLast = (int) spline[i + 1];
			}
		} else {
			// int canvasWidth = g.;
			// int canvasHeight
			...
			for (int i = 0; i < spline.length; i += 3) {
				int xLast = (int) spline[i];
				int yLast = (int) spline[i + 1];
				int zLast = (int) spline[i + 2];

				// //TODO: instead of painting a line, set the values here only if the zLast is
				// bigger than the previous value
				// //TODO: need to include all the points of the spread
				// //TODO: only if x/y are less than canvas size
				// if (xLast < canvasWidth && yLast < canvasHeight) {
				// zValues[xLast][yLast] = zLast;
				// }

				// TODO: the color should use the zAxis with a max/min so it looks even.
				Color c = new Color(Math.abs(zLast) % 255, Math.abs(zLast) % 255, Math.abs(zLast) % 255);
				g.setColor(c);
				if (spread > 0) {
					// Spread down
					g.drawLine(xLast, yLast, xLast, yLast + spread);
				} else {
					// if negative then spread right on the X
					g.drawLine(xLast, yLast, xLast + spread, yLast);
				}
			}
		}
	}

} // End of SplineFactory
