...
public class PCanvas3D extends Canvas3D {
	...
	public void initcanvas(SimpleUniverse universe) {
		...
		ArrayList<Point3f> verts = new ArrayList<Point3f>(6);
		Point3f o, p;
		...
		/*
		 * origin = new Shape3D(); Font3D font3D = new Font3D(new Font("Helvetica",
		 * Font.PLAIN, 1), new FontExtrusion()); Text3D tx = new Text3D(font3D, "X");
		 * tx.setPosition(new Point3f(1.0f, 0.0f, 0.0f)); origin.addGeometry(tx); Text3D
		 * ty = new Text3D(font3D, "Y"); ty.setPosition(new Point3f(0.0f, 1.0f, 0.0f));
		 * origin.addGeometry(ty); Text3D tz = new Text3D(font3D, "Z");
		 * tz.setPosition(new Point3f(0.0f, 0.0f, 1.0f)); origin.addGeometry(tz);
		 * origin.setName("ORIGINTX"); objRoot.addChild(origin);
		 */
